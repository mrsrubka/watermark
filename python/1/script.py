...
#watermark_width = width / K
#watermark_height = height / K
...
cv2.waitKey(0)
# http://stackoverflow.com/questions/7587490/converting-numpy-array-to-opencv-array

blank_image = np.zeros((K,K), np.uint8)
#blank_image = np.zeros((K,K))
# http://stackoverflow.com/questions/12881926/create-a-new-rgb-opencv-image-using-python
print "blank_image_BEFORE"
print blank_image
#blank_image[:,0.5*width:width] = (255)
...
print "blank_image_signed"
print blank_image_signed

resized_image = np.zeros((width,height), np.uint8)
#resized_image = np.zeros((width,height))
...
for i in range(width):
	for j in range(height):
		noise[i][j] = random.randint(0,2)-1
...
#img_dem = np.dot(new_img,noise_mark_signed)
img_dem = np.dot(dst,noise_mark_signed)
